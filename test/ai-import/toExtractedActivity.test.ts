...
describe('구간표 배치', () => {
  ...
  it('같은 종류의 표가 둘이면 첫 번째만 쓴다', () => {
    const r = toExtractedActivity(
      dto({
        distanceMeters: 2000,
        durationSeconds: 610,
        segmentSets: [
          { kind: 'lap', labelText: 'Laps', unitMeters: null, rows: [row(1, 1000, 312), row(2, 1000, 298)] },
          { kind: 'lap', labelText: 'Laps', unitMeters: null, rows: [row(1, 500, 150)] },
        ],
      }),
    );
